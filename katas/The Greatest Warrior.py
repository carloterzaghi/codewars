...
# Link for Kata: https://www.codewars.com/kata/5941c545f5c394fef900000c
...
class Warrior:
    ...
    def training(self, list_training):
        description, exp_gain, min_lv = list_training

        if self.level < min_lv:
            return "Not strong enough"
        
        else:
            self._achiev.append(description)
            if self._exp+exp_gain >10000:
                self._exp = 10000
            else:
                self._exp+=exp_gain
    ...
    def battle(self, enemy_lv):
        if enemy_lv > 100 or enemy_lv < 1:
            return "Invalid level"
        
        exp_gain = 0

        if self.level == enemy_lv:
            exp_gain = 10
        elif self.level-1 == enemy_lv:
            exp_gain = 5
        elif self.level < enemy_lv:
            if self.level+5 <= enemy_lv and self.rank != self.get_rank(enemy_lv):
                return "You've been defeated"
            else:
                diff = enemy_lv - self.level
                exp_gain = 20 * diff * diff
